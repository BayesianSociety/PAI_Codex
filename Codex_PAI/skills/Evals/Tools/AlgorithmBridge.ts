...
export async function updateISCWithResult(result: AlgorithmEvalResult): Promise<void> {
  ...
  await $`bun run /home/postnl/PAI_Codex/Codex_PAI/skills/THEALGORITHM/Tools/ISCManager.ts update --row ${result.isc_row} --status ${status} --note "${formatForISC(result)}"`.quiet();
}
